...
export default {
  async fetch(request: Request, env: unknown, ctx: unknown) {
    ...
    // One canonical form per page: apex host, https, no trailing slash.
    // Everything is normalised in a single hop so redirect chains stay short.
    ...
    // break the funnel's /api/path-review submissions.
    ...
    if (needsRedirect) {
      // Only force https on hosts we own. The Higgsfield-hosted preview and
